...
<body class="admin_config_group" >
   ...
   <div id="main" class="col-xs-12 col-sm-9 main" style="overflow-y: scroll;">
                                ...
                                <div class="col-xs-12 col-sm-12 clearfix">
                                    <form class="form" method="get" action="">
                                        <div class="form-group right">
											...
											<div style="width:100%; height:30px; margin-top:10px;"><a style="float:left;width:120px; text-align:center; padding:10px 15px; background:blue;color:#fff;" href="javascript:" onclick="daochu()">导出表格</a></div><br />
                                            <div class="sum">
                                            <div>会员总数:<?php echo ($count); ?></div>
                                            <!-- <div>激活总数:<?php echo ($count); ?></div> -->
                                            <div>余额总数:<?php echo ($yue_sum); ?></div>
                                            <div>分享佣金总数:<?php echo ($dt_sum); ?></div>
                                            <!-- <div>激活码总数:<?php echo ($active_sum); ?></div> -->
                                            <div>排单币总数:<?php echo ($paidan_sum); ?></div>
                                            <!-- <div>积分总数:<?php echo ($red_sum); ?></div> -->
                                            </div>
											<!-- <div style="width:150px;margin-right:20px;float:right;"><a href="javascript:" onclick="daochu()">导出表格</a></div> -->
                                        </div>
                                    </form>
                                </div>
                            </div>
                                                    ...
                                                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i; if(!($_SESSION["user_auth"]["uid"]<>1 && $data['yinbi']==1)){ ?>
                                                        <tr>
                                                            ...
                                                            <td style="white-space: nowrap;" >
                                                               ...
                                                               <br />
                                                               分享佣金：<span style="color:#dc3300" ><?php echo ($data['dt_jifen']+0); ?></span>
                                                               <br />
                                                               <!-- 激活码：<span style="color:#dc3300" ><?php echo ($data['active_num']+0); ?></span> -->
                                                               <!-- <br /> -->
                                                               排单币：<span style="color:#dc3300" ><?php echo ($data['paidan']+0); ?></span>
                                                               <br />
                                                               <!-- 积分：<span style="color:#dc3300" ><?php echo ($data['red_integral']+0); ?></span> -->
                                                            </td>
                                                            
                                                            <td><?php echo (date('Y-m-d H:i:s',$data['reg_date'])); ?></td>
                                                            <td>
																账号：<?php echo (get_parent_account($data["pid"])); ?> <br/>
																姓名：<?php echo M('user')->where('userid='.$data['pid'])->getField('username');?>
															</td>
                                                            <td>
                                                                <?php echo M('user')->where(array('pid'=>$data['userid']))->count(1); ?>人

                                                            </td>
															<td><?php echo ($data['teams_count']); ?></td>
															<td><?php echo ((isset($data[shouyi]) && ($data[shouyi] !== ""))?($data[shouyi]):"0"); ?></td>
															<td><?php echo ($data['zhouqi']); ?></td>
                                                            <td>
                                                                <?php if($data['is_paidan'] == 1): ?><span style="color:#0ae28c">已排单</span>
                                                                <?php else: ?>
                                                                    <span style="color:red">未排单</span><?php endif; ?>
                                                            </td>
															<td>
                                                                <?php if($data['activate'] == 1): ?><span style="color:#0ae28c">已激活</span>
                                                                <?php else: ?>
                                                                    <span style="color:red">未激活</span><?php endif; ?>
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                ...
                                                                <a name="edit" title="编辑" class="<?php if(!in_array(1,$hylb)) echo "bky"; ?>   label label-primary-outline label-pill" href="<?php echo U('User/edit',array('id'=>$data['userid']));?>">编辑</a>

                                                                <?php if(($data['status']) == "1"): ?><a name="forbid" title="锁定" class="<?php if(!in_array(3,$hylb)) echo "bky"; ?>   label label-warning-outline label-pill ajax-get confirm" href="<?php echo U('User/setStatus',array('status'=>forbid,'ids'=>$data['userid'],'model'=>'User'));?>">锁定</a>
                                                                <?php else: ?>
                                                                <a name="forbid" title="解锁" class="<?php if(!in_array(3,$hylb)) echo "bky"; ?>   label label-success-outline label-pill ajax-get confirm" href="<?php echo U('User/setStatus',array('status'=>resume,'ids'=>$data['userid'],'model'=>'User'));?>">解锁</a><?php endif; ?>
                                                                <div style="height:10px" ></div>

                                                                 <a name="edit" title="修改财富" class="<?php if(!in_array(2,$hylb)) echo "bky"; ?>   label label-primary-outline label-pill" href="<?php echo U('User/AddFruits',array('id'=>$data['userid']));?>">财富</a>
                                                                 ...
                                                                 <a title="删除" class="<?php if(!in_array(2,$hylb)) echo "bky"; ?> label label-primary-outline label-pill" onclick="Deluserid(this,'<?php echo ($data["userid"]); ?>')">删除</a>
                                                                 <a title="登录" target="_blank" class="<?php if(!in_array(5,$hylb)) echo "bky"; ?>   label label-primary-outline label-pill" href="<?php echo U('User/userlogin',array('userid'=>$data['userid']));?>">登录</a>
																<a href="javascript:" title="周期" class="<?php if(!in_array(2,$hylb)) echo "bky"; ?> label label-primary-outline label-pill" onclick="getcycle('<?php echo ($data["userid"]); ?>')">周期</a>
                                                        </tr>
    ...
    <div class="clearfix full-script">
	 ...
	 function getcycle(uid) {
        $.get("<?php echo U('User/getcycle');?>",{uid:uid},function(res){
            alert('周期：'+res);
        })
     }
     function Deluserid(obj,uid){
        var classname = $(obj).attr('class');
        if(classname=='bky label label-primary-outline label-pill'){
            return;
        }
        layer.prompt({title: '输入删除口令，并确认', formType: 1}, function(pass, index){
            $.get("<?php echo U('User/deluser');?>", {uid:uid,password:pass},function(res){
               if(res.status==1){
                    layer.msg(res.info);
                    location.reload();
               }else{
		...
		function daochu(){
			...
			window.location.href = '/admin/User/derivedExcel?date_start='+date_start+'&date_end='+date_end+'$type='+<?php echo ($type); ?>
		}
        $('.date').datetimepicker({
            format: 'yyyy-mm-dd',
            language:"zh-CN",
