   ...
   <div id="main" class="col-xs-12 col-sm-9 main" style="overflow-y: scroll;">
				...
				<div class="builder formbuilder-box">
					...
					<div class="builder-container" >
						...
						<!-- <div class="builder-toolbar">
							<div class="row">
								<div class="col-xs-8">
									<form action="<?php echo U('ActivateNum/add');?>" method="post" class="form-horizontal form form-builder">
									...
									<form class="form" method="get" action="">
										<div class="form-group right">
											<div class="input-group search-form">
												<input type="text" name="keyword" class="search-input form-control" value="<?php echo ($_GET["keyword"]); ?>" placeholder="关键字/时间">
											  ...
											  <thead>
												...
												<tbody>
													<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i; if($data['pay_no'] != ''): ?><tr>
															...
															<td>
																<?php if($data['my'] == 1): ?>我的资产
																<?php elseif($data['my'] == 2): ?>
																	分享佣金
																<?php elseif($data['my'] == 3): ?>
																	<span style="color:red;">抢单钱包</span>
																<?php elseif($data['my'] == 4): ?>
																	排单币收益<?php endif; ?>
															</td>
															<!-- <td>
															...
															<!-- <td>
																<a name="matching" title="匹配" class="label label-primary-outline label-pill confirm"  href="<?php echo U('Finance/pipei',array('id'=>$data['payout_id']));?>">匹配</a>
															
																<a name="demolition" title="拆单" class="label label-success-outline label-pill confirm"  href="<?php echo U('Finance/chaidan',array('id'=>$data['payout_id']));?>">拆单</a>
															
															
															
																<a name="delete" title="删除" class="label label-danger-outline label-pill ajax-get confirm"  href="<?php echo U('setStatus',array('ids'=>$data['id'],'status'=>'delete'));?>">删除</a>
															</td> -->
																<!--<td>
															<a name="matching" title="匹配" class="label label-primary-outline label-pill confirm"  href="<?php echo U('Finance/pipei',array('id'=>$data['id'],'payout_id'=>$data['payout_id']));?>">匹配</a>-->
																<!--<a name="delete" title="取消" class="label label-danger-outline label-pill ajax-get confirm"  href="<?php echo U('cancel',array('ids'=>$data['id']));?>">取消</a>-->
																<!-- <a name="demolition" title="拆单" class="label label-danger-outline label-pill confirm"  href="<?php echo U('Finance/chaidan',array('uid'=>$data['payout_id'],'id'=>$data['id']));?>">拆单</a> -->
															...
															<td>
																<?php if(in_array(2,$financials) || $auth_id==1){ ?>
															    	<a name="delete" title="取消" class="label label-danger-outline label-pill ajax-get confirm"  href="<?php echo U('cancel',array('ids'=>$data['id']));?>">取消</a>
																<?php }?>
															</td>
	...
	<script>
		function daochu(){
			...
			window.location.href = '/admin/Finance/sell_derivedExcel?date_start='+date_start+'&date_end='+date_end
		}
