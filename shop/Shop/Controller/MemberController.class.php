...

{
	...
	public function upshop(){
		$vip_grade = (int)$_GET['vip_grade'];
		$uid = session('userid');
		$red_integral = M('store')->where(array('uid'=>$uid))->getField('red_integral');
		
		if($vip_grade < 1){
			ajaxReturn('请激活会员',0);
		}elseif($red_integral < 100){
			ajaxReturn('需要拥有1000个红积分');
		}else{
			ajaxReturn('请填写资料',1,'/Shop/Member/verify');
		}
	}
