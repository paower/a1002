...
//		array('pid', '/^1[3456798]\d{9}$/', '推荐人手机错误', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
		...
		//验证用户名
		array('username', '', '该姓名已被使用', self::MUST_VALIDATE, 'unique', self::MODEL_BOTH),
...
//		array('resafety_pwd', 'safety_pwd', '两次输入交易密码不一致', self::MUST_VALIDATE, 'confirm', self::MODEL_INSERT),
    ...
    public function Quicklogin($account)
    {
		...
		$reg_time = $user_info['reg_date'];
		$news_time = time();
		$end_time = strtotime($reg_time) + 15*60*60*24;

		if($user_info['vip_grade'] == 0 && $news_time <= $end_time){
			$res = $this->where(array('userid'=>$user_info['userid']))->setField('status',0);
			if($res){
				$this->error = '您的体验账号超过15天未激活，已锁定，请联系管理员';
			}
			return false;
		}
