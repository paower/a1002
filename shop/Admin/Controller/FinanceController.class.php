...
class FinanceController extends AdminController
{
	...
	public function index()
	{
        ...
        if($auth_id<>1){
        	$auth_id    = M('admin')->where(array('id'=>$uids))->getField('auth_id');
			$financials    = explode(',',M('group')->where(array('id'=>$auth_id))->getField('financial'));
        }
		$this->assign(['financials'=>$financials,'auth_id'=>$auth_id]);
		$this->display();

	}
	public function cancel(){
		$id = I('ids');
		$orderinfo = M('trans')->where('id='.$id)->find();
		if($orderinfo['my']==1){
			$setname = 'cangku_num';
			$get_type = 114;
		}elseif($orderinfo['my']==4){
			$setname = 'paidan_profit';
			$get_type = 115;
		}elseif($orderinfo['my']==3){
			$setname = 'qiangdan_profit';
			$get_type = 116;
		}
		$inc = M('store')->where('uid='.$orderinfo['payout_id'])->setInc($setname,$orderinfo['pay_nums']);
		$yue = M('store')->where('uid='.$orderinfo['payout_id'])->getField($setname);
		$tranmoney = [
			'get_id'=>$orderinfo['payout_id'],
		...
		M('trans')->where('id='.$orderinfo['id'])->delete();
		$this->success('取消成功');
	}
    public function deltrans($id){
		if(M('trans')->delete($id)){
			$this->success('删除成功');
		}else{
			$this->error('删除失败');
		}
	...
	public function cannot(){
		...
		$data_list = $user_object
			...
			->order('form asc,yuyue_first_tail asc,id desc')->select();
	...
	public function pipei()
	{
		...
		$payout_id = I('get.payin_id');
		// 获取所有用户
		$map['status'] = array('egt', '0'); // 禁用和正常状态
		$map['trans_type'] = array('eq', '1'); 
		$map['pay_state'] = array('eq', '0');
		$map['payout_id'] = array('neq',$payout_id);
	...
	public function test(){
		...
		$where = 'id in ('.$str.')';
		$pay_nums = M('trans')->where($where)->field('id,pay_nums,sellout_num,payout_id,card_id,peo_num,purchase_num,form')->order('id desc')->select();
		...
		$sellout_num = M('trans')->where(array('id'=>$payout_id))->getField('purchase_num');
		
		// if($total_pay_nums>$apay_nums){
		// 	exit(json_encode(array('code'=>0,'msg'=>'所选金额大于卖出金额')));
		// }
		$payout_uid = M('trans')->where(array('id'=>$payout_id))->getField('payin_id');
		...
		// }
		$payin_info = M('trans')->where('id='.$payout_id)->field('form,paidan_num,out_card,yuyue_no,yuyue_first_tail')->find(); 
		foreach($pay_nums as $k => $v){
			...
			if($apay_nums > $v['pay_nums'] && $apay_nums != 0){
				...
				$v['peo_num'] = $peo_num + 1;
				$arr = ['purchase_num'=>$sellout_num,'payin_id'=>$payout_uid,'pay_state'=>1,'pipeitime'=>time(),'peo_num'=>$v['peo_num'], 'form'=>$payin_info['form'],'dopurs_time'=>$dopurs_time,'paidan_num'=>$payin_info['paidan_num'],'out_card'=>$payin_info['out_card'],'yuyue_first_tail'=>$payin_info['yuyue_first_tail'],'yuyue_no'=>$payin_info['yuyue_no']];

				$res = M('trans')->where(array('id'=>$v['id']))->save($arr);
				$arr2 = array('pay_nums'=>$apay_nums);
				$res2 = M('trans')->where(array('id'=>$payout_id))->save($arr2);
				// $payin_mobile = M('user')->where(array('userid'=>$v['payout_id']))->getField('mobile');
				...
				$arr2 = ['payout_id'=>$v['payout_id'],'pay_state'=>1,'pipeitime'=>time(),'sellout_num'=>$v['sellout_num'],'dopurs_time'=>$dopurs_time,'card_id'=>$v['card_id']];
				...
				if($apay_nums != 0){
					...
					$apay_nums = 0;
					$arr = ['pay_nums'=>$diff];
					$arr2 = ['sellout_num'=>$v['sellout_num'],'payout_id'=>$v['payout_id'],'pay_state'=>1,'pipeitime'=>time(),'dopurs_time'=>$dopurs_time,'card_id'=>$v['card_id']];
					$res = M('trans')->where(array('id'=>$v['id']))->save($arr);
					$res2 = M('trans')->where(array('id'=>$payout_id))->save($arr2);
					// $payin_mobile = M('user')->where(array('userid'=>$v['payout_id']))->getField('mobile');
					// $this->newMsg($payin_mobile);
	...
	public function cannot_derivedExcel(){
        ...
        public function completeNew_derivedExcel(){
			$add_time=I('get.date_start');
			$end_time=I('get.date_end');
			$add_time=empty($add_time)?0:strtotime($add_time);
			$end_time=empty($end_time)?0:strtotime($end_time);
